...
const config = {
  ...
  /**
   * @returns {int} Отдает количество набранных очков.
   */
  getScore() {
...
const snake = {
  ...
  getNextStepHeadPoint() {
    ...
    switch (this.direction) {
      case 'up':
        firstPoint.y === 0 ? objRet = {x: firstPoint.x, y: this.config.getRowsCount() - 1} : objRet = {x: firstPoint.x, y: firstPoint.y - 1};
        return objRet;
      case 'down':
        firstPoint.y === 20 ? objRet = {x: firstPoint.x, y: 0} : objRet = {x: firstPoint.x, y: firstPoint.y + 1};
        return objRet;
      case 'right':
        firstPoint.x === 20 ? objRet = {x: 0, y: firstPoint.y} : objRet = {x: firstPoint.x + 1, y: firstPoint.y};
        return objRet;
      case 'left':
      firstPoint.x === 0 ? objRet = {x: this.config.getColsCount() - 1, y: firstPoint.y} : objRet = {x: firstPoint.x - 1, y: firstPoint.y};
        return objRet;
    }
  },
...
const game = {
  ...
  tickHandler() {
    ...
    // // Если следующий шаг будет за границу поля
    // if (!this.nextStepNoAbroad()) {
    //   const direction = snake.getLastStepDirection(); // получаем направление змейки
    ...
    //   switch (direction) {
    //     case `up`:
    //       this.snake.makeStep({x: nextStep.x, y: this.config.getRowsCount() - 1});
    //       break;
    //     case `down`:
    ...
    if (this.food.isOnPoint(this.snake.getNextStepHeadPoint())) {
      ...
      this.config.settings.score++;
      // отрисовываем счет
      document.querySelector('#score').textContent = this.config.getScore();
  ...
  /**
   * Проверяет уйдет ли змейка за границу поля.
   */
  nextStepNoAbroad() {
    // Получаем следующую точку головы змейки в соответствии с текущим направлением.
    const nextHeadPoint = this.snake.getNextStepHeadPoint();
